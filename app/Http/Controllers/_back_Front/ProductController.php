                          ...
                          <li>
                            <div class="product-container">
                                <div class="left-block">
                                    ...
                                    if($product->sale_price>0) {
                                        $output .=  '<div class="price-percent-reduction2">
...
//                exit;
                ...
                $data['product_discount'] = '<span class="discount-price">'.SM::productDiscount($product_id).'%</span>';
                $data['product_regular_price'] = '<del class="regular-price">'.SM::product_price(SM::productRegularPrice($product_id)).'</del>';
                $data['product_price'] = '<span class="product-current-price">'.SM::product_price($product_price).'</span>';
            ...
            if ($customers) {
                foreach ($customers as $cKey => $color) {
                    ...
                    if ($activeColor == $color->color_id) {
                        ...
                        $checked = "checked";
                        $attribute_price = $color->attribute_price;
                ...
                $data['product_discount'] = '<span class="discount-price">'.SM::productDiscount($product_id).'%</span>';
                $data['product_regular_price'] = '<del class="regular-price">'.SM::product_price(SM::productRegularPrice($product_id)).'</del>';
                $data['product_price'] = '<span class="product-current-price">'.SM::product_price($attribute_price).'</span>';
                $data['attribute_measurement'] = $attribute_legnth . $attribute_front . $attribute_back . $attribute_chest;
                $data['attribute_label'] = $output;
                return Response()->json($data);
            } else {
    ...
    private function getProductSearchData($searchtext)
    {
        ...
        if (count($product_data) > 0) {
            $output .= '<div class="container">
                           <ul class="row product-list miain-search-product grid g-3"> ';
            foreach ($product_data as $product) {
                if ($product->product_type == 2) {
                    $att_data = SM::getAttributeByProductId($product->id);
                    if (!empty($att_data->attribute_image)) {
